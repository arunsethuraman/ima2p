...
void setup_mapping(char *fname)// JH fixed signficiant bug in this functino 4/5/2010 
{
  ...
  while (fgets(modeltextline,MAXMODELTEXTLINE,nestedmodelfile)!= NULL) if (isalpha(modeltextline[0]))
  {
    ...
    if (strcmp(keyword,"equal")==0 || (strcmp(keyword,"constant")==0)) 
    {
      ...
      if (m_or_p == 'p')
        n = fillplist(c,&plist[numm][0], 0, paramsnotused[inmodel]);
      if (m_or_p == 'm')
        n = fillplist(c,&plist[numm][0],numpopsizeparams,paramsnotused[inmodel]);
      for (i=0;i<n;i++)
      {
        if (m_or_p == 'p')
...
void setbounds()
{
  ...
  for (i = 0; i < nparams; ++i) 
  {
    ...
    {
      parameter_upper_bound[i] = itheta[i].pr.max;
    }
    else
    {
      parameter_upper_bound[i] = imig[i-numpopsizeparams].pr.max;
    }
  }
} /* setbounds */
