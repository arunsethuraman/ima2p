...
/* revised extensively from earlier IM programs on 4/23/07 */
/*********** LOCAL STUFF **********/
         ...
         int nomigration)
{
  ...
  if (p[0] != p[1])             // add 1 migration
  {
    ...
    if (uniform () < 0.5)
    {
      checkmig (j, &(gtree[c[0]].mig), &(gtree[c[0]].cmm));
      ...
      {
        assert (gtree[c[0]].mig[j].mt < C[ci]->tvals[lastperiodnumber]);
      }
    ...
    {
      checkmig (j, &(gtree[c[1]].mig), &(gtree[c[1]].cmm));
