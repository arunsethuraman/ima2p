...
// VS - add details about this function
// this function will increase the size of the arrays used to store miginfo
// we give as input the pointer to the structure and the new size of the array
// INPUT
//    int n: the new size of the array
//    struct edge *gtree : pointer to a structure edge
// Details
//    the variable gtree->cmm stores the size of the migration array
//    the function will check if n>gtree->cmm. If so, it reallocates memory 
//    to size n.
void
checkmigt (int n, struct edge *gtree)
{
  ...
  if (n + 2 > gtree->cmm)
  {
    ...
    /* CR:110412.1 
     *  VS - 03/11/2011 potential bug detected here!
     *  If MIGINC is small we may end up not allocating enough memory.
     */
    cmm += IMAX(MIGINC, n+2-cmm);

    if (cmm > ABSMIGMAX)
    {
      IM_err(IMERR_MIGARRAYTOOBIG," too many migrations in array %d",cmm);
    }
    *mig = static_cast<struct migstruct *> 
            (realloc (*mig, cmm * sizeof (struct migstruct)));
    gtree->cmm = cmm;
  }
  return;
}
...
//    if i is larger than nmig it means that we need to increase 
...
checkmig (int i, struct migstruct **mig, int *nmig)
{
  ...
  if (i + 2 > *nmig)
  {
    /* CR:110412.1 
     *  VS - 03/11/2011 potential bug detected here!
     *  because the realloc could fail to allocate enough memory
     */
    ...
    *mig = static_cast<struct migstruct *> 
            (realloc (*mig, *nmig * sizeof (struct migstruct)));
  }
}
...
/*  CR 110825.1   Function not currently being used.  Expected use in future
 */
